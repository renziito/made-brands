...
class CategoryTranslations extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('category_id, language_id, name, created_at, updated_at', 'required', 'message' => '{attribute} no debe estar vacio.'),
			array('category_id, language_id', 'length', 'max'=>10),
			array('name', 'length', 'max'=>255),
			array('name_size, description_size', 'length', 'max'=>20),
			array('description', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, category_id, language_id, name, name_size, description, description_size, created_at, updated_at', 'safe', 'on'=>'search'),
		);
	}
