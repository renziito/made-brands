...
$baseCatalogUrl = $this->createUrl('productos');

$totalProducts = (int) $dataProvider->getTotalItemCount();
$currentProducts = $dataProvider->getData();
$pagination = $dataProvider->getPagination();

$firstItem = $totalProducts > 0
	? ((int) $pagination->getCurrentPage() * (int) $pagination->getPageSize()) + 1
	: 0;

$lastItem = $totalProducts > 0
	? min(
		$firstItem + count($currentProducts) - 1,
		$totalProducts
	)
	: 0;
...
$catalogUrl = function ($params = array()) use ($baseCatalogUrl) {
...
$categoryUrl = function ($category) use ($catalogUrl) {

	return $catalogUrl(array(
...
/*
 * Build a category slug lookup for subcategory URLs.
 */
...
$makeSubcategoryUrl = function ($subcategory) use (
	$catalogUrl,
	$categorySlugById
) {
	...
	if (isset($categorySlugById[$categoryId])) {
		$params['categoria'] =
...
$clearFiltersUrl = $catalogUrl();


$brandUrl = function ($brand) use ($catalogUrl) {
...
/*
 * ==========================================================
 * FILTERED CATEGORY / SUBCATEGORY STATE
 * ==========================================================
 */
...
foreach ($categories as $category) {
	...
	if (
		...
		(int) $category['id'] === (int) $selectedCategoryId
	) {
...
$fullSheetUrl = trim(
	(string) WebUtils::getSiteSetting('full_sheet')
);
...
$initialProductImage =
	$selectedProduct !== null
	? $imageUrl($selectedProduct['infographic_image'])
	: '';
	...
	<div class="catalog-layout">
		...
		<aside
			...
			<div class="catalog-sidebar__header">
				...
				<span><?= WebUtils::getMenuItemByKey('filters', $languageId)['label']  ?></span>
			...
			<div
				...
				<button
					...
					<span><?= WebUtils::getMenuItemByKey('categories', $languageId)['label']  ?></span>
				...
				<div class="catalog-filter-section__content">
					...
					<?php foreach ($categories as $category): ?>
						...
						<div
							...
							<div class="catalog-filter-category__row">
			...
			<div
				...
				<button
					...
					<span><?= WebUtils::getMenuItemByKey('brands', $languageId)['label']  ?></span>
			...
			<?php if (
				...
				<a
					...
					<?= WebUtils::getMenuItemByKey('clear_filters', $languageId)['label'] ?>
			...
			<?php if ($fullSheetUrl !== ''): ?>
				...
				<a
					...
					href="<?= CHtml::encode($fullSheetUrl); ?>"
					target="_blank"
					rel="noopener noreferrer">

					<i class="fas fa-external-link-alt"></i>

					<?= WebUtils::getMenuItemByKey('download_product_catalog', $languageId)['label'] ?>
		...
		<main class="catalog-content">
			...
			<button
				...
				<?= WebUtils::getMenuItemByKey('show_filters', $languageId)['label'] ?>
			...
			<div class="catalog-heading">
				...
				<div class="catalog-heading__copy">
					...
					<p class="catalog-eyebrow">
						<?= WebUtils::getMenuItemByKey('our_products', $languageId)['label']  ?>
					</p>
				...
				<div class="catalog-heading__tools">
					...
					<div class="catalog-count">
						...
						<?php if ($totalProducts > 0): ?>
							...
							<?= WebUtils::getMenuItemByKey('showing', $languageId)['label'] ?>
							<strong><?= $firstItem; ?>–<?= $lastItem; ?></strong>
							<?= WebUtils::getMenuItemByKey('of', $languageId)['label'] ?>
							<strong><?= $totalProducts; ?></strong>
							<?= WebUtils::getMenuItemByKey('products', $languageId)['label'] ?>

						<?php else: ?>

							0 <?= WebUtils::getMenuItemByKey('products', $languageId)['label'] ?>
						...
						<select
							...
							<option
								...
								<?= $orderFilter === 'recientes'
									? 'selected'
									: ''; ?>>
								<?= WebUtils::getMenuItemByKey('sort_most_recent', $languageId)['label']  ?>
							</option>
							...
							<option
								...
								<?= $orderFilter === 'nombre'
									? 'selected'
									: ''; ?>>
								<?= WebUtils::getMenuItemByKey('sort_name', $languageId)['label']  ?>
							</option>
							...
							<option
								...
								<?= $orderFilter === 'antiguos'
									? 'selected'
									: ''; ?>>
								<?= WebUtils::getMenuItemByKey('sort_oldest', $languageId)['label'] ?>
			...
			<?php if (
				...
				<div class="catalog-active-filters">
					...
					<?php if ($selectedCategoryName !== ''): ?>
						...
						<span class="catalog-active-filter">
					...
					<?php if ($selectedSubcategoryName !== ''): ?>
			...
			<?php if (!empty($currentProducts)): ?>
							...
							<button
								...
								<div class="catalog-product__body">
									...
									<?php if ($brandName !== ''): ?>
										...
										<p class="catalog-product__brand">
									...
									<h2 class="catalog-product__name">
				...
				<?php if (
					$dataProvider->getPagination()->getPageCount() > 1
				): ?>
			...
			<?php else: ?>
				...
				<div class="catalog-empty">
					...
					<h2 class="catalog-empty__title">
						...
						<?= WebUtils::getMenuItemByKey('no_products_found', $languageId)['label'] ?>

					</h2>

					<p class="catalog-empty__text">

						<?= WebUtils::getMenuItemByKey('remove_filters_message', $languageId)['label'] ?>
					...
					<a
						...
						<?= WebUtils::getMenuItemByKey('view_all_products', $languageId)['label'] ?>
...
<div
	...
	<div class="catalog-modal__dialog">
		...
		<div class="catalog-modal__loading">

			<?= WebUtils::getMenuItemByKey('loading_product_information', $languageId)['label'] ?>

		</div>


		<div class="catalog-modal__image-wrap">
			...
			<div
				...
				<?= WebUtils::getMenuItemByKey('product_infographic_unavailable', $languageId)['label'] ?>
...
$initialProductJson = CJSON::encode(
	array(
		'id' => $initialProductId,
...
(function () {
	...
	var modal =
		document.getElementById("catalog-product-modal");

	var modalClose =
		document.getElementById("catalog-modal-close");

	var modalImage =
		document.getElementById("catalog-modal-image");

	var modalPlaceholder =
		document.getElementById("catalog-modal-placeholder");

	var mobileFilterButton =
		document.getElementById("catalog-mobile-filter-button");

	var sidebar =
		document.getElementById("catalog-sidebar");

	var orderSelect =
		document.getElementById("catalog-order");
	...
	function renderModal(productId, productSlug, productName, imageUrl, updateHistory) {
		...
		function () {
			...
			var matchingCard =
				document.querySelector(
					".catalog-product[data-product-slug=\"" +
					CSS.escape(productSlug) +
					"\"]" 
				);
	...
	if (
		...
		var initialCard =
			document.querySelector(
				".catalog-product[data-product-id=\"" +
				initialProduct.id +
				"\"]" 
			);
